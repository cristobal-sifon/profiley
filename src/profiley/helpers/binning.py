...
class Bin2D:
    ...
    def bin(self, data2d, weights=None, err=False, get_count=False, mask_nan=False):
        ...
        if weights is None:
            ...
            res = (
                np.bincount(self.digitized[keep], data2d.reshape(-1)[keep])[1:-1]
                / count
            )
            if err:
                ...
                std = (
                    np.bincount(self.digitized[keep], diff[keep])[1:-1]
                    / (count * (count - 1))
                ) ** 0.5
        else:
            count = np.bincount(self.digitized, weights.reshape(-1))[1:-1]
            res = (
                np.bincount(self.digitized, (data2d * weights).reshape(-1))[1:-1]
                / count
            )
        if get_count:
